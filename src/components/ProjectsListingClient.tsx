...
export const ProjectsListingClient: React.FC = () => {
      ...
      <div className="relative flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-14 w-full">
        ...
        <div className="max-w-3xl mb-12">
          ...
          <p className="text-slate-700 dark:text-gray-300 text-base sm:text-lg leading-relaxed">
            A showcase of my full-stack web applications, freelancing client solutions, and technical case studies. Click <strong>Explore Project</strong> on any project to view its detailed architectural breakdown.
          </p>
        </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                ...
                <div className={`${proj.featured && proj.image ? "lg:col-span-7" : "lg:col-span-12"} space-y-4`}>
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {/* FREELANCING PROJECT BADGE */}
                    {proj.isFreelance && (
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 dark:bg-emerald-950 border border-emerald-300 dark:border-emerald-700/60 text-emerald-900 dark:text-emerald-300 text-xs font-bold uppercase tracking-wider shadow-sm">
                        <Briefcase className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />
                        <span>Freelancing Project</span>
                      </span>
                    )}

                    {proj.featured && !proj.isFreelance && (
                  ...
                  {/* ACTION BUTTONS INCLUDING EXPLORE PROJECT & LIVE DEMO */}
                  <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-slate-200 dark:border-emerald-900/40">
                    ...
                    {proj.detailUrl && (
                      <Link
                        href={proj.detailUrl}
                        onClick={() => trackProjectExplore(proj.id)}
                ...
                {proj.featured && proj.image && proj.detailUrl && (
                  <div className="lg:col-span-5 flex justify-center">
                    <Link
                      href={proj.detailUrl}
                      onClick={() => trackProjectExplore(proj.id)}
                      className="group block relative w-full overflow-hidden rounded-xl border border-slate-200 dark:border-emerald-800/40 shadow-xl hover:border-emerald-400/60 transition-all duration-300"
                    >
                      <Image
                        src={proj.image}
                        alt={`${proj.title} developed by Ashrith Namburi`}
                        width={600}
                        height={380}
